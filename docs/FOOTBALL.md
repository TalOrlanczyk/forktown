...
This is a deterministic animated match, not a physics engine or user-controlled sports game. It has no tackles, fouls, offside decisions, injuries, extra time, persistent league, or multiplayer. The UTC calendar date and match number seed every possession. The same six-match lineup repeats in each 24-minute town day throughout that UTC date, with a new lineup at midnight UTC. The UTC clock chooses the match and its exact elapsed time, so viewers with agreeing device clocks see the same score, ball, players, commentary, and sound cues regardless of timezone, reloads, or arrival time. No per-visit randomness or accumulated simulation state is involved. Both the interactive town and live broadcast use this same function. Pause freezes the view; resuming rejoins the live match. Reduced-motion preferences start the town paused.
